...
type Props = {
  ...
  className?: string;
  emptyHint?: string;
  /** When true, clicking the hero image opens a full-screen lightbox (with prev/next when multiple). */
...
export function CampaignImageCarousel({
    ...
    <>
      ...
      {enableLightbox ? (
        <button
          type="button"
          title="View larger"
          aria-label="View campaign image larger"
  ...
  if (n === 1) {
    return (
      <>
        <div className={`relative overflow-hidden bg-amber-100 ${aspectClass} ${className}`}>{mainImage}</div>
