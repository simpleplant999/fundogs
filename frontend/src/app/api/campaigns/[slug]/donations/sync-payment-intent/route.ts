...
import { syncDonationStripePaymentIntent } from "@/server/donations/campaign-donations";
...
export async function POST(request: Request, context: Ctx) {
  ...
  if (!body?.paymentIntentId || body.paymentIntentId.length < 10) {
    return jsonError("paymentIntentId is required", 400);
  }
  try {
    return jsonOk(await syncDonationStripePaymentIntent(slug, body.paymentIntentId));
  } catch (e) {
    if (e instanceof CampaignHttpError || e instanceof PaymentHttpError) {
      return jsonError(e.message, e.status);
    }
    console.error(e);
    return jsonError("Failed to sync payment intent", 500);
  }
}
