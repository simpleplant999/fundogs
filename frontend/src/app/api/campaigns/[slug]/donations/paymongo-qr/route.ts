...
import { createPaymongoQrDonation } from "@/server/donations/campaign-donations";
...
export async function POST(request: Request, context: Ctx) {
  ...
  try {
    return jsonOk(
      await createPaymongoQrDonation(slug, {
    ...
    console.error(e);
    return jsonError("Failed to create PayMongo QR donation", 500);
  }
}
