...
const UPLOAD_SUBDIR = join('uploads', 'organizations');

const IMAGE_EXT = /\.(jpe?g|png|gif|webp|bmp|heic|heif|avif)$/i;

export function orgImagesFileFilter(
  _req: unknown,
...
export function orgImagesMulterStorage() {
  return diskStorage({
...
export function publicOrgImageUrl(
  ...
  return `${safeProto}://${host}/uploads/organizations/${filename}`;
}
